console.log(
  '%cEntLibrary %cedited by %cthoratica %cfor %cEntryBlockers/blocks.\n%cOriginal code by %cmuno9748.',
  'font-size: 1.2rem; font-family: sans-serif; font-weight: bold; color: #4f80ff;',
...
Entry.CustomBlock = class EntryBlock {
  constructor(type) {
    ...
    this.meta_ = {
      ...
      class: 'default',
      func: () => {
        console.warn("Error! Block's Action is not defined!")
      },
  ...
  setColor(_default, darken, text) {
    _default &&
      /#[0-9a-fA-F]{6}/.test(_default) &&
      (this.meta_.color = _default)
  ...
  setParams(params) {
    ...
    params.forEach((param, i) => {
      ...
      this.meta_.paramsKeyMap[param.name] = i
    })
    return this
  }
  ...
  mutate() {
    ...
    if (!window.useWebGL) {
      Entry.Mutator.mutate(
        this.type,
        { template: this.meta_.template, params: this.meta_.params },
        this.meta_
      )
  ...
  return {
    mutate() {
      ...
      if (!window.useWebGL) {
        ...
        Entry.getMainWS()
          .blockMenu.view.find('.entryCategoryElementWorkspace')[0]
          .click()
      } else console.warn('Working on play mode... This will have some bugs')
    },
  }
...
Entry.CustomCategory = class EntryCategory {
  ...
  addBlock(...blockType) {
    Entry.staticBlocks
      .find((c) => c.category == this.categoryName)
...
let addItem = (title, desc, img, link) => {
  Entry.addEventListener('openExpansionBlockManager', () => {
    document.querySelector('div.extend_block__a65d5 ul.list__a65d5').append(
      ((code) => {
        let t = document.createElement('template')
        t.innerHTML = code
        return t.content.cloneNode(true)
      })(`
    <li class="addedItem" onclick="this.classList.toggle('on__a65d5')">
      <div class="link__a65d5">
         <div class="thmb__a65d5" style="background-image: url('${img}'); background-repeat: no-repeat; position: relative;"></div>
         <div class="inner_box__a65d5">
            <strong class="sjt__a65d5">${title}</strong>
            <div class="dsc__a65d5">${desc}</div>
...
Entry.Command[218].do = (blockNames) => {
  Entry.dispatchEvent('addBlock')
...
Entry.addEventListener('addBlock', () => {
